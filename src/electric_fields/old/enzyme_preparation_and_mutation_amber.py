...
def prepare_and_mutate_enzyme(enzyme_name, element_active_site):
    ...
    """
    reactive_residue_name = clean_pdb(enzyme_name)
...
# TODO: find the histidines that need to be corrected 
# TODO: Histidine next to Mo should be HIE, the histidine close to S should be HIP
def correct_protonation_enzyme(protein_matrix_csv, cofactor_csv, input_file, output_file):
    ...
    for i,row1 in his_n.iterrows():
        for j,row2 in df_s.iterrows():
            dist = np.sqrt((row1['x'] - row2['x'])**2 + (row1['y'] - row2['y'])**2 +(row1['z'] - row2['z']) ** 2)
            if dist <= 4:
                print((row1['chain_id'], int(row1['position'])))
                print(change_to_hie)
...
def protonate_enzyme(enzyme_name, input_file, ph=7):
    ...
    """
    output_pqr = os.path.join(enzyme_name, f'{enzyme_name}_mutated_protonated.pqr')
    output_pdb = os.path.join(enzyme_name, f'{enzyme_name}_mutated_protonated.pdb')
